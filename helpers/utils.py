...
def process_structural_alerts(df: pd.DataFrame, smiles_column: str) -> pd.DataFrame:
    ...
    df["structure_filter_alerts"] = [process_filters(x) for x in results["reasons"].values]
    return df
